...
// interface.  
// 2001: This file is the same as the provided ix_test.cc file, except
//       for ensuring that values used for SlotNum take into account
//       that SlotNum could be a short int. 
...
#define NENTRIES     2010             // Size of values array
...
RC InsertStringEntries(IX_IndexHandle &ih, int nEntries)
{
   ...
   for (i = 0; i < nEntries; i++) {
     ...
     RID rid(values[i] + 1, (SlotNum)((values[i] + 1)*2));
...
RC VerifyIntIndex(IX_IndexHandle &ih, int nStart, int nEntries, int bExists)
{
   ...
   for (i = nStart; i < nStart + nEntries; i++) {
      ...
      if (bExists && rc == 0) {
         ...
         if (pageNum != value || slotNum != (SlotNum)(value*2)) {
